...
ALTER TABLE salary 
CHANGE COLUMN `Years of Experience` experience INT;

ALTER TABLE salary 
CHANGE COLUMN `Education Level` education varchar(255);

ALTER TABLE salary 
CHANGE COLUMN `Job Title` job varchar(255);

-- Let's see how many jobs data do we have 
SELECT DISTINCT job from salary;
...
Select job ,  experience , Salary 
From salary 
Where salary >= 50000 AND job = 'Data Analyst'
ORDER by experience;   
...
SELECT Age , job , experience , Salary 
FROM salary 
Where experience = 2 AND job = 'Software Engineer' 
ORDER BY Age ;
...
select age , job , experience , Salary 
From salary 
Where job  LIKE '%manager' AND experience <= 2 
ORDER BY Salary DESC;
...
select age , job , experience , Salary 
From salary 
Where job  LIKE '%VP of%' OR  job LIKE '%Chief%' OR job = 'CEO' 
ORDER BY Salary DESC;
...
Select DISTINCT(job) , count(job) as CountOfJob
from salary 
where salary <= 50000
group by job;
...
select age , job , experience , Salary 
From salary 
Where job  LIKE '%Marketing%' OR job  LIKE '%HR%' 
ORDER BY Salary ;
...
SELECT DISTINCT education from salary;

-- Average , Lowest and highest salary earned by people owning various degrees 
 
SELECT education , avg(salary) AS averageHighschoolerSalary , MAX(salary) AS maximumsalary , MIN(salary) AS LowestSalary
from salary 
...
select education , job , salary 
from salary 
where education Like "%Bachelor's%" AND job LIKE '%Engineer%' 
ORDER BY salary DESC ; 


select education , job , salary 
from salary 
where education Like "%Bachelor's%" AND job = 'Marketing Coordinator' 
ORDER BY salary DESC ; 
...
select education , job , salary 
from salary 
where education Like "%Master's" OR education like "%Master's Degree%" AND job LIKE '%Engineer%' 
ORDER BY salary ; 

 -- Let's compare an engineers who has done masters with someone with low paying jobs with masters 
Select education , job , salary 
from salary 
where (education = "Master's" OR education = "Master's Degree") AND job = 'Marketing Coordinator' 
ORDER BY salary ; 
...
SELECT education, job, salary 
FROM salary 
WHERE (education = "Master's" OR education = "Master's Degree") AND job = 'Sales Representative' 
ORDER BY salary;
...
Select education , count(education) as MoresalaryMoreExperience
 from salary 
 where experience >=10 AND salary >= 150000
 group by education;
...
Select education , count(education) as MoresalaryMoreExperience
 from salary 
 where experience >=10 AND salary >= 150000 AND job Like '%Sales%'
 group by education;
...
Select education , count(education) as MoresalaryMoreExperience
 from salary 
 where experience >=10 AND salary >= 150000 AND job Like '%Engineer%'
 group by education;
...
Select age , job , salary , experience
from salary
...
Select Gender, Count(Gender) AS WorkingInEngineering
from salary 
Where  job LIKE '%Engineer%' AND age<=25
GROUP BY Gender;
...
Select Gender , Count(Gender) AS HigherEarning
From Salary
Where salary >=100000 
Group by Gender;
...
Select Gender , Count(Gender) AS HigherEarning
From Salary
Where salary >=100000 AND education LIKE "%Bachelor's%" AND experience >= 5 
Group by Gender; 
...
Select Gender, Count(Gender) AS WorkingInEngineering
from salary 
Where  (job LIKE '%Marketing%' OR job LIKE '%Sales%') AND experience >= 5
GROUP BY Gender;
...
Select Gender , AVG(salary) as AverageEarning
from salary 
Where (education LIKE "%Bachelor's%" OR education LIKE "%Master's%") AND (job LIKE "%Engineer%" OR job LIKE "%Analyst%")
GROUP BY Gender;
...
Select Gender , AVG(salary) as AverageEarning
from salary 
Where (education LIKE "%Bachelor's%" OR education LIKE "%Master's%") AND (job LIKE "%Sales%" OR job LIKE "%Marketing%")
GROUP BY Gender;

-- okay it seems like there is a income parity between men and women even in tech or non tech even though there are more women high earners in non-tech the average  is lower. 
-- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 
-- This was the SQL code for the beggining of the Portofolio project. Call it a final draft. I started in 10am and its 1:13 am the next day. 
-- This might seem like nothing much but this is all my efforts and we are moving into next step of the project. 
-- This is a first draft of the code and I will change this later so, if this is than you are one hell of a lucky dude or duddet. 
-- And if you are an interviewer, I am pretty skilled with SQL, much more than i've shown in this code. Wait for the  second part of the project man. 
